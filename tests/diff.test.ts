...
describe("differs", () => {
	it("is false when every incoming value already matches", () => {
		assert.equal(differs({ steps: 8000, other: 1 }, { steps: 8000 }), false);
	});

	it("is true when a value changed", () => {
		assert.equal(differs({ steps: 8000 }, { steps: 8100 }), true);
	});

	it("is true when the property is new", () => {
		assert.equal(differs({}, { steps: 8000 }), true);
	});
	...
	it("compares workout lists by value, not identity", () => {
		const existing = { workouts: [{ name: "Run", minutes: 31 }] };
		assert.equal(differs(existing, { workouts: [{ name: "Run", minutes: 31 }] }), false);
		assert.equal(differs(existing, { workouts: [{ name: "Run", minutes: 32 }] }), true);
		assert.equal(
			differs(existing, { workouts: [{ name: "Run", minutes: 31 }, { name: "Ride" }] }),
			true,
		);
	});

	it("does not treat 0 and null as equal", () => {
		assert.equal(differs({ steps: null }, { steps: 0 }), true);
	});

	it("notices a string that became a number", () => {
		assert.equal(differs({ steps: "8000" }, { steps: 8000 }), true);
	});
});
